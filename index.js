...
async function main({
  inicio = 0,
  fim = tamanho_da_lista_de_acoes,
  tickers = acoes_disponiveis,
}) {
  try {
    ...
    buscarDados(slice_tickers).then((dados) => {
      const htmlElementsList = stockModel(dados).map((acao) =>
        acao.convertToHTMLCard()
      );
...
brapi.getAvailableStocks().then((tickers_list) => {
  //Chama a função main() apenas com os tickers disponíveis na API
  acoes_disponiveis = tickers_list;
  console.log("chamando funcao aqui");
  main({});
});
